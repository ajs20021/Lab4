...
``` r
lab4data <- read.csv("C:/Users/ajsut/Documents/GitHub/Lab4/skewed.csv")
```
...
``` r
ggplot(lab4data, aes(x = Performance)) + geom_histogram(binwidth = 20)+ theme_light()
```
...
Not normal because the p-value is not above .05. They also looked
visually non-normal. The histogram particularly looks positively skewed.
...
The result of the levene test was significant meaning that the equal
variance assumption was violated across groups. Also, the variances are
more than double in size between groups.
...
ggplot(lab4data, aes(x = Performance_log))+ geom_histogram(binwidth = .2)+ theme_light()
```
...
Performance log column under lab4data represents transformed data. When
I substitute the above performance variables for performance_log, I see
that the plots look more normal than before. The histogram looks much
more normal than it did before log transformation. After transformation,
the histogram no longer looks skewed.
